  ...
  return (
    <div className="bg-white p-8 rounded-3xl border border-emerald-900 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-0 mb-8">
        ...
        {/* Timeframe Buttons - Desktop (md and up) */}
        <div className="hidden md:flex gap-2 bg-gray-100 p-2 rounded-full">
          {(["Day", "Month", "Year"] as const).map((tf) => (
            <button
              key={tf}
              onClick={() => setTimeframe(tf)}
        ...
        {/* Timeframe Dropdown - Mobile (below md) */}
        <div className="md:hidden w-full sm:w-auto flex gap-2 bg-gray-100 p-2 rounded-full">
          {(["Day", "Month", "Year"] as const).map((tf) => (
            <button
              key={tf}
              onClick={() => setTimeframe(tf)}
              className={`flex-1 sm:flex-none px-6 py-2 rounded-full font-semibold transition-all ${
                timeframe === tf
                  ? "bg-orange-500 text-white"
                  : "text-gray-600 hover:text-gray-900"
              }`}
